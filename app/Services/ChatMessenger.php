...
class ChatMessenger
{
    ...
    {
        // dd('working', 'push', $channel, $event, $data);
    ...
    public function parseMessage($prefetchedMessage = null, $id = null)
    {
        ...
        return [
            ...
            'from_id' => $msg->from_id,
            'to_id' => $msg->to_id,
            'from_user' => $msg->fromUser->first_name,
            'from_user_avatar' => $this->getUserWithAvatar($msg->fromUser)->avatar,
            'message' => $msg->body,
            'attachment' => (object) [
                'file' => $attachment,
    ...
    public function fetchMessagesQuery($user_id)
    {
        ...
        return Message::where('from_id', Auth::user()->id)->where('from_deleted', false)->where('to_id', $user_id)
            ->orWhere('from_id', $user_id)->where('to_id', Auth::user()->id)->where('to_deleted', false);
    }
    ...
    public function deleteConversation($user_id)
    {
        try {
            $userId = Auth::id();
            Message::where('from_id', $userId)->update(['from_deleted' => 1]);
            Message::where('to_id', $userId)->update(['to_deleted' => 1]);
    ...
    public function getUserAvatarUrl($user_avatar_name)
    {
        return get_storage_image(config('chat.user_avatar.folder'), $user_avatar_name, 'user');
    }
