...
class BlockedCoachDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($item) {
                ...
                if (auth()->user()->can('Approve Coach')) {
                    $buttons .= '<form action="'.route('admin.coach.approve', $item->id).'"  id="accept-form-'.$item->id.'" method="post" style="">
                    <input type="hidden" name="_token" value="'.csrf_token().'">
                    <input type="hidden" name="_method" value="POST">
                    <button class="dropdown-item text-success" onclick="return makeAcceptRequest(event, '.$item->id.')"  type="submit" title="Unblock"><i class="ri-thumb-up-fill"></i> Unblock</button></form>';
                }
    ...
    {
        return $model->newQuery()->where('status', BLOCK)->with('user')->orderBy('id', 'DESC');
    }
