...
class ApproveTeacherDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($item) {
                ...
                if (auth()->user()->can('Block Teacher')) {
                    $buttons .= '<form action="'.route('admin.teacher.block', $item->id).'"  id="accept-form-'.$item->id.'" method="post" style="">
                    <input type="hidden" name="_token" value="'.csrf_token().'">
                    <input type="hidden" name="_method" value="POST">
                    <button class="dropdown-item text-warning" onclick="return makeAcceptRequest(event, '.$item->id.')"  type="submit" title="Block"><i class="ri-error-warning-fill"></i> Block</button></form>';
                }
                if (auth()->user()->can('Login Teacher')) {
    ...
    {
        return $model->newQuery()->where('status', APPROVED)->with('user')->orderBy('id', 'DESC');
    }
