...
class AllCoursesDataTable extends DataTable
{
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($item) {
                $buttons = '';
                if (auth()->user()->can('Approve Course') && $item->status !== APPROVED) {
                ...
                if (auth()->user()->can('Hold Course') && $item->status !== HOLD) {
                    $buttons .= '<a class="dropdown-item text-warning" href="'.route('admin.course.status.change', [$item->id, HOLD]).'" title="Hold"><i class="ri-flag-2-fill"></i> Hold </a>';
                }
            ...
            ->editColumn('course_category_id', function ($item) {
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('user_id', 'user_id')->title('Teacher'),
            Column::make('course_category_id', 'course_category_id')->title('Course Category'),
    ...
    {
        return 'Admin\AllCourses_'.date('YmdHis');
    }
}
