...
class ApiPlanDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($item) {
                $buttons = '';
                if (auth()->user()->can('Edit Api Plan')) {
                    $buttons .= '<a class="dropdown-item" href="'.route('admin.business.api-plans.edit', $item->id).'" title="Edit"><i class="mdi mdi-square-edit-outline"></i> Edit </a>';
                }

                if (auth()->user()->can('Delete Api Plan')) {
                    $buttons .= '<form action="'.route('admin.business.api-plans.destroy', $item->id).'"  id="delete-form-'.$item->id.'" method="post" style="">
     ...
     */
    public function query(ApiPlan $model)
    {
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('call_limit', 'call_limit')->title('Call Limit'),
    ...
    {
        return 'Admin\ApiPlan_'.date('YmdHis');
    }
}
