...
class EbookCategoryDataTable extends DataTable
{
    /**
        ...
        return datatables()
            ...
            ->addColumn('action', function ($item) {
                ...
                if (auth()->user()->can('Edit Pre-Requisition')) {
                    $buttons .= '<a class="dropdown-item" href="'.route('admin.ebook-categories.edit', $item->id).'" title="Edit"><i class="mdi mdi-square-edit-outline"></i> Edit </a>';
                }

                if (auth()->user()->can('Delete Pre-Requisition')) {
                    $buttons .= '<form action="'.route('admin.ebook-categories.destroy', $item->id).'"  id="delete-form-'.$item->id.'" method="post" style="">
     ...
     */
    public function query(EbookCategory $model)
    {
    ...
    {
        return 'Admin\EbookCategory_'.date('YmdHis');
    }
}
