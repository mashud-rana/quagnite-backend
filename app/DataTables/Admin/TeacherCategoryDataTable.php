...
class TeacherCategoryDataTable extends DataTable
{
    /**
        ...
        return datatables()
            ...
            ->addColumn('action', function ($item) {
                ...
                if (auth()->user()->can('Edit Pre-Requisition')) {
                    $buttons .= '<a class="dropdown-item" href="'.route('admin.teacher-categories.edit', $item->id).'" title="Edit"><i class="mdi mdi-square-edit-outline"></i> Edit </a>';
                }

                if (auth()->user()->can('Delete Pre-Requisition')) {
                    $buttons .= '<form action="'.route('admin.teacher-categories.destroy', $item->id).'"  id="delete-form-'.$item->id.'" method="post" style="">
     ...
     */
    public function query(TeacherCategory $model)
    {
    ...
    {
        return 'Admin\TeacherCategory_'.date('YmdHis');
    }
}
