...
class EnrollBootcampStudentDataTable extends DataTable
{
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('bootcamp', function ($item) {

                return $item->bootcamp->title;
            })
            ->filterColumn('bootcamp', function ($query, $keyword) {
                $query->orWhereHas('bootcamp', function ($subQuery) use ($keyword) {
                    $subQuery->where('title', 'like', "%{$keyword}%");
                });
            })
            ->rawColumns(['action', 'bootcamp', 'status'])->addIndexColumn();
    }
     ...
     */
    public function query(EnrollBootcamp $model)
    {
        return $model->newQuery()->with(['student', 'bootcamp'])->orderBy('id', 'DESC');
    }
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('bootcamp', 'bootcamp')->title('Bootcamp'),
