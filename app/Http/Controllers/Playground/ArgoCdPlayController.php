...
class ArgoCdPlayController extends Controller
{
    protected $region;
    protected $domain;
    ...
    #!/bin/bash
    su -s /bin/bash -c 'minikube start && sleep 10 && kubectl port-forward svc/argocd-server -n argocd --address 0.0.0.0 8090:80' mcdns
    SCRIPT;
    ...
    {
        set_page_meta('Argo CD Playground');
        return view('frontend.playground.argocd.argocd-playground');
    }
    ...
    public function provisionLab()
    {
        ...
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_ARGOCD)) {
        ...
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_ARGOCD)) {

            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_ARGOCD);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_ARGOCD);
        ...
        $dataArray['playgroundType'] = PlAYGD_ARGOCD;
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_ARGOCD)->first();
