...
class GrafanaPlayController extends Controller
{
    protected $region;
    protected $domain;
    ...
    #!/bin/bash
    docker run -d -p 8080:8080 -p 9090:9090 -p 3000:3000 nahidul88/pg_play
    SCRIPT;
    ...
    {
        set_page_meta('Grafana Prometheus Playground');
        return view('frontend.playground.grafana.grafana-playground');
    }
    ...
    public function provisionLab()
    {
        ...
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_GRPR)) {
        ...
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_GRPR)) {

            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_GRPR);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_GRPR);
        ...
        $dataArray['playgroundType'] = PlAYGD_GRPR;
        ...
        // $dataArray['subDomain'] = '9090-' . $subDomain;
        ...
        // $dataArray['subDomain'] = '3000-' . $subDomain;

        // if (!$this->playgroundService->createRoute53Record($dataArray)) {
        //     return $this->playgroundService->serviceNotAvailable();
        // }

        //Create Default dns record
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_GRPR)->first();
