...
class JenkinsPlayController extends Controller
{
    protected $region;
    protected $domain;
    ...
    {
        set_page_meta('Jenkins Playground');
        return view('frontend.playground.jenkins.jenkins-playground');
    }
    ...
    public function provisionLab()
    {
        ...
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_JENKINS)) {
        ...
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_JENKINS)) {

            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_JENKINS);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_JENKINS);
        ...
        $dataArray['playgroundType'] = PlAYGD_JENKINS;
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_JENKINS)->first();
