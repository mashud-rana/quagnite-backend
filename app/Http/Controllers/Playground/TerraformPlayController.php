...
class TerraformPlayController extends Controller
{
    protected $region;
    protected $domain;
    ...
    {
        set_page_meta('Terraform Playground');
        return view('frontend.playground.terraform.terraform-playground');
    }
    ...
    public function provisionLab()
    {
        ...
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_TERRAFORM)) {
        ...
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_TERRAFORM)) {

            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_TERRAFORM);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_TERRAFORM);
        ...
        $dataArray['playgroundType'] = PlAYGD_TERRAFORM;
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_TERRAFORM)->first();
