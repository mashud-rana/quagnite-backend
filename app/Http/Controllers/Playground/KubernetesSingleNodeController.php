...
class KubernetesSingleNodeController extends Controller
{
    protected $region;
    protected $domain;
    ...
    #!/bin/bash
    su -s /bin/bash -c 'minikube start' mcdns
    SCRIPT;
    ...
    {
        set_page_meta('Kubernetes Single Node Playground');
        return view('frontend.playground.kubernetes.k8ssn-playground');
    }
    ...
    public function provisionLab()
    {
        ...
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_K8SSN)) {
        ...
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_K8SSN)) {

            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_K8SSN);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_K8SSN);
        ...
        $dataArray['playgroundType'] = PlAYGD_K8SSN;
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_K8SSN)->first();
