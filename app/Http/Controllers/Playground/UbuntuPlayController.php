...
class UbuntuPlayController extends Controller
{
    protected $region;
    protected $domain;
    ...
    #!/bin/bash
    docker run -d -p 8080:8080 nahidul88/ubuntu_play
    SCRIPT;
    ...
    {
        set_page_meta('Ubuntu Playground');
        return view('frontend.playground.ubuntu.ubuntu-playground');
    }
    ...
    public function provisionLab()
    {
        ...
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_UOS2004)) {
        ...
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_UOS2004)) {

            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_UOS2004);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_UOS2004);
        ...
        $dataArray['playgroundType'] = PlAYGD_UOS2004;
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_UOS2004)->first();
