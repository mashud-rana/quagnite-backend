...
class PwsPlayController extends Controller
{
    protected $region;
    protected $domain;
    ...
    #!/bin/bash
    docker run -d -p 8080:8080 nahidul88/powershell
    SCRIPT;
    ...
    {
        set_page_meta('Powershell Playground');
        return view('frontend.playground.powershell.powershell-playground');
    }
    ...
    public function provisionLab()
    {
        ...
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_PWS)) {
        ...
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_PWS)) {

            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_PWS);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_PWS);
        ...
        $dataArray['playgroundType'] = PlAYGD_PWS;
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_PWS)->first();
