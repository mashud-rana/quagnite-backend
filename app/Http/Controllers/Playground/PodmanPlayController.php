...
class PodmanPlayController extends Controller
{
    protected $region;
    protected $domain;
    ...
    #!/bin/bash
    su -s /bin/bash -c 'cd /home/mcdns' mcdns
    SCRIPT;
    ...
    {
        return view('frontend.playground.podman.podman-playground');
    }

    public function provisionLab()
    {
        // check Playground active or not
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_PODMAN)) {
        ...
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_PODMAN)) {

            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_PODMAN);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_PODMAN);
        ...
        $dataArray['playgroundType'] = PlAYGD_PODMAN;
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_PODMAN)->first();
