...
class DockerPlayground extends Controller
{
    protected $region;
    protected $domain;
    ...
    {
        return view('frontend.playground.docker.docker-playground');
    }

    public function provisionLab()
    {
        // check Playground active or not
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_DOCKER)) {
        ...
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_DOCKER)) {

            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_DOCKER);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_DOCKER);
        ...
        $dataArray['playgroundType'] = PlAYGD_DOCKER;
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_DOCKER)->first();
