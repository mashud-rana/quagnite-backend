...
class MavenPlayController extends Controller
{
    protected $region;
    protected $domain;
    ...
    #!/bin/bash
    docker run -d -p 8080:8080 nahidul88/maven_play
    SCRIPT;
    ...
    {
        set_page_meta('Maven Playground');
        return view('frontend.playground.maven.maven-playground');
    }
    ...
    public function provisionLab()
    {
        ...
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_MAVEN)) {
        ...
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_MAVEN)) {

            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_MAVEN);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_MAVEN);
        ...
        $dataArray['playgroundType'] = PlAYGD_MAVEN;
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_MAVEN)->first();
