...
class KubernetesMultiNodeController extends Controller
{
    protected $region;
    protected $domain;
    ...
    #!/bin/bash
    su -s /bin/bash -c 'minikube start --nodes 2 -p multinode' mcdns
    SCRIPT;
    ...
    {
        set_page_meta('Kubernetes Multi Node Playground');
        return view('frontend.playground.kubernetes.k8smn-playground');
    }
    ...
    public function provisionLab()
    {
        ...
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_K8SMN)) {
        ...
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_K8SMN)) {

            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_K8SMN);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_K8SMN);
        ...
        $dataArray['playgroundType'] = PlAYGD_K8SMN;
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_K8SMN)->first();
