...
class ChefPlayController extends Controller
{
    protected $region;
    protected $domain;
    ...
    #!/bin/bash
    docker run -d -p 8080:8080 nahidul88/chef_play
    SCRIPT;
    ...
    {
        set_page_meta('Chef Playground');
        return view('frontend.playground.chef.chef-playground');
    }
    ...
    public function provisionLab()
    {
        ...
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_CHEF)) {
        ...
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_CHEF)) {

            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_CHEF);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_CHEF);
        ...
        $dataArray['playgroundType'] = PlAYGD_CHEF;
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_CHEF)->first();
