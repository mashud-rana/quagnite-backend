...
class HelmController extends Controller
{
    ...
    {
        set_page_meta('Helm Playground');
        return view('frontend.playground.helm.helm-playground');
    }
    ...
    public function provisionLab()
    {
        ...
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_HELM)) {
        ...
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_HELM)) {

            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_HELM);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_HELM);
        ...
        $dataArray['playgroundType'] = PlAYGD_HELM;
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_HELM)->first();
