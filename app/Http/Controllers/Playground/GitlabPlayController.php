...
class GitlabPlayController extends Controller
{
    protected $region;
    protected $domain;
    ...
    #!/bin/bash
    docker run -d -p 8070:8080 nahidul88/ubuntu_play
    SCRIPT;
    ...
    {
        set_page_meta('Gitlab Playground');
        return view('frontend.playground.gitlab.gitlab-playground');
    }
    ...
    public function provisionLab()
    {
        ...
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_GITLAB)) {
        ...
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_GITLAB)) {

            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_GITLAB);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_GITLAB);
        ...
        $dataArray['playgroundType'] = PlAYGD_GITLAB;
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_GITLAB)->first();
