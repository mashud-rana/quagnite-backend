...
class AnsiblePlayController extends Controller
{
    protected $region;
    protected $domain;
    ...
    {
        set_page_meta('Ansible Playground');
        return view('frontend.playground.ansible.ansible-playground');
    }
    ...
    public function provisionLab()
    {
        ...
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_ANSIBLE)) {
        ...
        // check if already initialized the playgroundupdateLabInfo
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_ANSIBLE)) {

            // $lab['url'] = $labInfo->lab_url;
            // $lab['refresh'] = true;
            // $lab['instructions'] = $this->playgroundService->getPlaygroundInstructions(PlAYGD_ANSIBLE);
            // // check remaining duration
            // $now = Carbon::now();
            // if ($now->lessThan($labInfo->end_time)) {
            //     $lab['duration'] = Carbon::now()->diffInSeconds($labInfo->end_time);
            // } else {
            //     $lab['duration'] = 0;
            // }
            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_ANSIBLE);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_ANSIBLE);
        ...
        $dataArray['playgroundType'] = PlAYGD_ANSIBLE;
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_ANSIBLE)->first();
