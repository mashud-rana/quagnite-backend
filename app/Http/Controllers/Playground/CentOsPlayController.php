...
class CentOsPlayController extends Controller
{
    protected $region;
    protected $domain;
    ...
    #!/bin/bash
    docker run -d -p 8080:8080 nahidul88/centos8_play
    SCRIPT;
    ...
    {
        return view('frontend.playground.centos.centos-playground');
    }

    public function provisionLab()
    {
        // check Playground active or not
        if (!$this->playgroundService->checkPlaygroundIsActive(PlAYGD_COS8)) {
        ...
        if ($labInfo = $this->playgroundService->getUserOccupiedLab(PlAYGD_COS8)) {

            $lab = $this->playgroundService->getLabDetails($labInfo, PlAYGD_COS8);
            return response()->json(['message' => 'success', 'status' => 200, 'lab' => $lab]);
        }

        //Get and check Playground configurations Settings
        $dataArray = $this->playgroundService->getPlaygroundSettings(PlAYGD_COS8);
        ...
        $dataArray['playgroundType'] = PlAYGD_COS8;
    ...
    public function endLab(Request $request)
    {
        ...
        $lab = PlaygroundLab::whereUserId(auth()->id())->where('type', PlAYGD_COS8)->first();
