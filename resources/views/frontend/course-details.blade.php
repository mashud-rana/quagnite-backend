            ...
            <div class="ic__instructor-div">
                <div class="d-flex mb-4">
                    <div class="flex-shrink-0">
                        <img src="{{ asset('/frontend') }}/frontend/images/instructor-img.png"
                            class="img-fluid img-avatar" alt="...">
                    </div>
    ...
    <section class="ic-course-details-rating">
        <div class="reserve-container">
            ...
            <div class="ic-buttons-rating">
                @if ($alReadyEnrolled)
                    <button class="ic-btn btn-course-disable" disabled type="submit"
                        title="enroll">{{ $enrolled->status === GC::ENROLL_COURSE_STATUS_COMPLETE ? 'Completed' : 'Already Enrolled' }}
                    </button>
                @else
                    <form action="{{ route('frontend.save.enroll.course', $item->id) }}"
                        id="enroll-form-{{ $item->id }}" method="get">
                        @csrf
                        @method('GET')

                        <button class="ic-btn" onclick="return makeCourseEnrolRequest(event, '{{ $item->id }}')"
                            type="submit" title="enroll">Enroll Now <i class="ri-arrow-right-line"></i>
                        </button>
                    </form>
                @endif
            </div>
