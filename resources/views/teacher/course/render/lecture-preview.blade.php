...
@elseif ($lecture->lecture_format === LECTURE_FORMAT_VIMEO)
    ...
    @else
        <div>
            <h6 class="text-center">Please Wait, Video is still processing...</h6>
        </div>
    @endif
@elseif ($lecture->lecture_format === LECTURE_FORMAT_PDF)
    <embed src="{{ getStoreFile($lecture->pdf_path, $lecture->storage_type) }}" type="application/pdf" width="100%"
        height="600px">
@elseif ($lecture->lecture_format === LECTURE_FORMAT_SLIDE)
    <iframe
        src='https://view.officeapps.live.com/op/embed.aspx?src={{ getStoreFile($lecture->slide_path, $lecture->storage_type) }}'
        width='100%' height='600px' frameborder='0'>
    @elseif ($lecture->lecture_format === LECTURE_FORMAT_IMAGE)
        <img src="{{ getStoreFile($lecture->image_path, $lecture->storage_type) }}" alt="" width="100%">
