            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.teachers.update', $user->id) }}" method="POST"
                        ...
                        <div class="mb-3">
                            <label class="form-label mb-2 w-100">Teacher Category <span class="error">*</span></label>
                            <select class="form-control" name="teacher_category_id">
                                @foreach ($teacherCategory as $item)
                                    <option value="{{ $item->id }}" @if ($user->teacher->teacher_category_id == $item->id) selected @endif>
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                            @error('teacher_category_id')
                                <p class="error">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-check form-check-inline">
                                <input type="radio" id="status_yes" name="status" class="form-check-input"
                                    ...
                                    @if (ACTIVE == $user->status) checked @endif>
                            ...
                            <div class="form-check form-check-inline">
                                <input type="radio" id="status_no" name="status" class="form-check-input"
                                    ...
                                    @if (INACTIVE == $user->status) checked @endif>
                        ...
                        <div class="mb-3">
                            <div>
                                ...
                                @can('All Admins')
                                    <a class="btn btn-secondary waves-effect" href="{{ route('admin.teachers.index') }}">
