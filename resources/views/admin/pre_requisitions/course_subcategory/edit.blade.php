...
@section('content')
    <div class="row">
        <div class="col-6">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.course-subcategories.update', $item->id) }}" method="post"
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control select2" name="course_category_id" required>
                                @forelse ($course_categories as $cat)
                                    <option value="{{ $cat->id }}"
                                        {{ $item->course_category_id == $cat->id ? ' selected' : '' }}>{{ $cat->name }}
                                    </option>
                                @empty
