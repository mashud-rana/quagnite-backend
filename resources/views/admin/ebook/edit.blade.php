                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.ebook.update', $ebook->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input name="author_name" type="text" class="form-control" required=""
                                placeholder="Author Name" value="{{ old('author_name') ?? $ebook->author_name }}">
                            @error('author_name')
                                <p class="error">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            <select class="form-control select2" name="ebook_category_id">
                                ...
                                @foreach ($ebookCategories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ $category->id == $ebook->ebook_category_id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control select2" name="ebook_type">
                                ...
                                <option value="{{ EBOOK_TYPE_PDF }}"
                                    {{ $ebook->ebook_type === EBOOK_TYPE_PDF ? 'selected' : '' }}>PDF</option>
                                <option value="{{ EBOOK_TYPE_AUDIO }}"
                                    {{ $ebook->ebook_type === EBOOK_TYPE_AUDIO ? 'selected' : '' }}>Audio Book</option>
                                <option value="{{ EBOOK_TYPE_VIDEO }}"
                                    {{ $ebook->ebook_type === EBOOK_TYPE_VIDEO ? 'selected' : '' }}>Video Book</option>
                            </select>
                            @error('ebook_type')
                                <p class="error">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <div id='img_contain'>
                                <img id="image-preview" align='middle' src="{{ getStoreFile($ebook->image, $ebook->storage_type) }}"
                    ...
                    <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input id="inputTrix" type="hidden" name="details" value="{{ $ebook->details }}">
